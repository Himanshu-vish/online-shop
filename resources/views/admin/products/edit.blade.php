            ...
            <div class="col-md-4">
                ...
                <div class="card mb-3">
                    <div class="card-body">	
                        ...
                        <div class="mb-3">
                           <select multiple class="related_product w-100" name="related_product[]" id="related_product">
                              @if (!empty($related_Products))
                                  @foreach ($related_Products as $rel_product)
                                      <option selected value="{{$rel_product->id}}">{{$rel_product->title}}</option>
                                  @endforeach
                              @endif
                           </select>
                        </div>
...
<script>
 ...
 $("#productForm").submit(function(event){
    ...
    $.ajax({
        url: '{{route("products.update",$product->id)}}',  // Make sure the route is correct
        type: 'put',
        data: formArray,
        dataType: 'json',
...
$('#category').change(function(){
    ...
    $.ajax({
        url: '{{route("product-subcategories.index")}}',  // Make sure the route is correct
        type: 'get',
